...
from .runtime import HajiRuntime
...
class NotificationService:
    ...
    def notify(self, notification: Notification) -> list[Any]:
        ...
        return [handler(notification) for handler in handlers]
    ...
    def attach(self, runtime: HajiRuntime) -> None:
        runtime.on("agent.message", lambda payload: self.emit("حاجي", str(payload.get("text", payload.get("message", ""))), metadata=payload))
